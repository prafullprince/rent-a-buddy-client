...
"use client";
import { eventSummaryOfUser, markAsActiveInactive } from "@/service/apiCall/event.api";
...
const MyEvents = () => {
  ...
  async function onlineOffline(){
    try {
      await markAsActiveInactive(!eventDetails?.isActive, session?.serverToken, eventDetails?._id);
    ...
    <div>
      ...
      <div className="mt-4 bg-black/10 rounded-lg shadow-lg p-4 max-w-fit">
        <div className="flex flex-col gap-1">
          ...
          <button onClick={onlineOffline} className="px-4 cursor-pointer py-2 bg-yellow-500 text-black rounded-lg font-semibold flex items-center gap-2 hover:scale-95 transition-all duration-200 w-full mt-2">
            {!eventDetails?.isActive ? <MdOnlinePrediction className="text-2xl text-black" /> : <IoCloudOffline className="text-2xl text-black" />}
